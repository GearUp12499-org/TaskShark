...
internal fun getCaller(): String {
    try {
        ...
        for (frame in stack) {
            if (frame.className.contains("dev.aether.tcapi")) continue
            return "${
...
@JvmOverloads constructor(private val throwDebuggingErrors: Boolean = true) : SchedulerOld() {
    ...
    private val tasks: MutableMap<Int, ITaskOld> = mutableMapOf()
    ...
    protected fun selectState(state: State): List<ITaskOld> {
...
//                        println("locks: $locks")
    ...
    protected fun release(task: ITaskOld, cancel: Boolean = false) {
...
//            println("$task released $lock")
    ...
    protected fun displayTaskNoStatus(task: ITaskOld, indent: Int, writeLine: (String) -> Unit) {
    ...
    fun displayStatus(
        ...
        val notStartedList: MutableList<ITaskOld> = mutableListOf()
        val inProgressList: MutableList<ITaskOld> = mutableListOf()
        val finishedList: MutableList<ITaskOld> = mutableListOf()
        val cancelledList: MutableList<ITaskOld> = mutableListOf()
        var waiting = 0
        var progress = 0
        var done = 0
        var cancelled = 0
    ...
    override fun <T : ITaskOld> add(t: T): T {
        t.scheduler = this
        t.register()
        t.name = getCaller()
        return t
...
//            println("dropped ${dropped.size} tasks: ${dropped.joinToString(", ")}")
    ...
    override fun filteredStop(predicate: (ITaskOld) -> Boolean, cancel: Boolean) = filteredStop(
        predicate,
        cancel = cancel, dropNonStarted = false
    )

    override fun filteredStop(predicate: (ITaskOld) -> Boolean) = filteredStop(
        predicate,
        cancel = true,
