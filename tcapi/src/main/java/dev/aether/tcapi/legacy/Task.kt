...
import dev.aether.tcapi.legacy.ITaskOld.State
...
typealias TaskQuery2<T> = (task: Task, scheduler: SchedulerOld) -> T
...
class Task(
    ...
    internal var canStart: TaskQuery2<Boolean> = { _: Task, _: SchedulerOld -> true }
    internal var onStart: TaskAction2 = { _: Task, _: SchedulerOld -> }
    private var onTick: TaskAction2 = { _: Task, _: SchedulerOld -> }
    internal var isCompleted: TaskQuery2<Boolean> = { _: Task, _: SchedulerOld -> false }
    private var onFinish: TaskAction2 = { _: Task, _: SchedulerOld -> }
    private var then: TaskAction2 = { _: Task, _: SchedulerOld -> }
    ...
    fun canStart(block: TaskQuery1<Boolean>) {
        canStart = { that: Task, _: SchedulerOld -> block(that) }
    }

    fun canStart(block: Producer<Boolean>) {
        canStart = { _: Task, _: SchedulerOld -> block() }
    }
    ...
    fun onStart(block: TaskAction1) {
        onStart = { that: Task, _: SchedulerOld -> block(that) }
    }

    fun onStart(block: Runnable) {
        onStart = { _: Task, _: SchedulerOld -> block() }
    }
    ...
    fun onTick(block: TaskAction1) {
        onTick = { that: Task, _: SchedulerOld -> block(that) }
    }

    fun onTick(block: Runnable) {
        onTick = { _: Task, _: SchedulerOld -> block() }
    }
    ...
    fun isCompleted(block: TaskQuery1<Boolean>) {
        isCompleted = { that: Task, _: SchedulerOld -> block(that) }
    }

    fun isCompleted(block: Producer<Boolean>) {
        isCompleted = { _: Task, _: SchedulerOld -> block() }
    }
    ...
    fun onFinish(block: TaskAction1) {
        onFinish = { that: Task, _: SchedulerOld -> block(that) }
    }

    fun onFinish(block: Runnable) {
        onFinish = { _: Task, _: SchedulerOld -> block() }
    }
